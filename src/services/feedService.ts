  ...
  static async hydrateReelsWithViewerState(reels: any[], viewerId?: string): Promise<ReelWithDetails[]> {
    ...
    return reels.map((reel) => ({
      ...
      user: {
        ...
        profilePicUrl: reel.user?.profilePicUrl || null,
        isVerified: reel.user?.isVerified || false,
        category: reel.user?.category || 'Personal',
      },
      sound: reel.sound || null,
      hashtags: Array.isArray(reel.hashtags)
        ? reel.hashtags.map((h: any) => (h && h.hashtag ? h.hashtag.tag : (typeof h === 'string' ? h : ''))).filter(Boolean)
        : [],
